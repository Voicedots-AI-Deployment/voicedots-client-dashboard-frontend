...
export default function CreateDriveWizard({programs,drive,draftId,collegeTimezone='UTC',onCancel,onSaved}:{programs:Program[];drive?:Drive|null;draftId?:string|null;collegeTimezone?:string;onCancel:()=>void;onSaved:(message:string)=>void}) {
  ...
  useEffect(()=>{
    ...
    const timer=setTimeout(()=>collegeApi.get<string[]>(`drives/role-suggestions?q=${encodeURIComponent(form.role_title)}`,controller.signal).then(setRoleSuggestions).catch(()=>{}),180);
  ...
  async function generate(track:string){
    setGenerating(track);setFormError('');
    ...
    catch(error){applyBackendErrors(error);}finally{setGenerating('');}
  }
  ...
  function validate(index:number):Record<string,string>{
    ...
    if(index===1){
      ...
      else if(form.window_start&&startInstant<=Date.now())errors.window_start='Interview start must be in the future.';
  ...
  function apiPayload(){
    return {
      ...
      drive_date:dateToApi(form.drive_date),application_deadline:dateToApi(form.application_deadline),window_start:form.window_start,window_end:form.window_end,
  ...
  function inputField(title:string,key:keyof FormState,type='text',required=false,placeholder=''){
    return <label className="block text-sm font-medium" key={key}>{label(title,required)}<input id={key} className={`${input} ${fieldErrors[key]?'border-rose-500 focus:border-rose-500 focus:ring-rose-100':''}`} type={type} value={form[key]} placeholder={placeholder} list={key==='role_title'?'drive-role-options':undefined} aria-invalid={Boolean(fieldErrors[key])} aria-describedby={fieldErrors[key]?`${key}-error`:undefined} onChange={event=>update(key,event.target.value as never)}/>{key==='role_title'&&<datalist id="drive-role-options">{roleSuggestions.map(role=><option key={role} value={role}/>)}</datalist>}{fieldError(key)}</label>;
  }
    ...
    <main className={`${panel} dw-main`}>
      ...
      {step===0&&<div className="dw-content space-y-5">
        ...
        <Section title="Role details" description="These details belong to this drive, even when you reuse a company profile.">
          <div className="grid gap-4 md:grid-cols-2">{inputField('Role','role_title','text',true,'e.g. Data Analyst')}{selectField('Job type','job_type',[['full_time','Full Time'],['internship','Internship'],['internship_plus_ppo','Internship + PPO']],true)}{inputField('Location','location','text',true,'City, state or remote')}<p className="self-end text-xs text-slate-500">Role suggestions appear as you type. Selecting one never copies old drive settings.</p></div>
        </Section>
      ...
      {step===2&&<div className="dw-content space-y-5">
        <Section title="Interview sequence" description="Select up to four roles. Their order determines the sequence. Persona names are system-managed.">
          ...
          {Boolean(library?.agents.length)&&<label className="block max-w-xl text-sm font-medium">{label('Use a saved custom interview role')}<select className={input} value="" onChange={event=>{const agent=library?.agents.find(item=>item.id===event.target.value);if(!agent)return;setSelection(current=>{if(current.some(item=>item.agent_id===agent.id))return current;const index=current.findIndex(item=>!item.agent_id);if(index<0)return current;return current.map((item,position)=>position===index?{...item,agent_id:agent.id||null}:item);});}}><option value="">Choose a saved role to use an interview slot</option>{library?.agents.filter(agent=>!selection.some(item=>item.agent_id===agent.id)).map(agent=><option key={agent.id} value={agent.id}>{agent.role}</option>)}</select><small className="mt-1 block text-xs font-normal text-slate-500">Only the interview role order is configured here; persona avatar and voice identities remain system-managed.</small></label>}
      ...
      {step===3&&<div className="dw-content space-y-4">{selection.map((item,index)=>{
        ...
        return <Section key={item.track} title={`Round ${index+1} · ${roleName(item)}`} description="Choose the question source for this role.">
          ...
          {round.question_source==='personalized'?<p className="dw-info">Questions will be created for the candidate at interview time. They are not fixed in advance.</p>:<div className="dw-fixed-questions"><div className="flex flex-wrap items-center justify-between gap-2"><p className="text-sm text-slate-600 dark:text-slate-300">Saved questions will be asked in the order shown.</p>{round.question_source==='ai_generated'&&<button type="button" className={button} disabled={generating===round.track||!form.jd_text.trim()} onClick={()=>void generate(round.track)}>{generating===round.track?'Generating…':'Generate from job description'}</button>}</div>{round.questions.map((question,qindex)=><div className="dw-question-row" key={`${round.track}-${qindex}`}><span>{qindex+1}</span><textarea aria-label={`${roleName(item)} question ${qindex+1}`} id={`question-${round.track}-${qindex}`} className={input} rows={2} value={question} onChange={event=>setRounds(all=>all.map(row=>row.track===round.track?{...row,questions:row.questions.map((text,pos)=>pos===qindex?event.target.value:text)}:row))}/><div className="flex gap-1"><button type="button" className={button} aria-label="Move question up" disabled={qindex===0} onClick={()=>moveQuestion(round.track,qindex,-1)}><ArrowUp size={15}/></button><button type="button" className={button} aria-label="Move question down" disabled={qindex===round.questions.length-1} onClick={()=>moveQuestion(round.track,qindex,1)}><ArrowDown size={15}/></button><button type="button" className={button} aria-label="Remove question" onClick={()=>setRounds(all=>all.map(row=>row.track===round.track?{...row,questions:row.questions.filter((_,pos)=>pos!==qindex)}:row))}><Trash2 size={15}/></button></div></div>)}<button type="button" className={button} onClick={()=>addQuestion(round.track)}><Plus size={15}/>Add question</button>{fieldError(`questions.${round.track}`)}</div>}
        </Section>;
      })}</div>}
      ...
      {step===4&&<div className="dw-content space-y-5">
        ...
        <Section title="Graduation year range" description="Use the range handles for quick selection or enter exact years.">
          <div className="dw-year-range"><div className="flex items-center justify-between"><label>From <input id="graduation_from" className={input} type="number" value={selectedYears.length?yearMin:''} placeholder={String(availableYears[0]||'')} min={availableYears[0]} max={availableYears.at(-1)} onChange={event=>changeYearRange(Number(event.target.value),yearMax)}/></label><span className="dw-range-line" aria-hidden="true"/><label>To <input id="graduation_to" className={input} type="number" value={selectedYears.length?yearMax:''} placeholder={String(availableYears.at(-1)||'')} min={availableYears[0]} max={availableYears.at(-1)} onChange={event=>changeYearRange(yearMin,Number(event.target.value))}/></label></div><div className="grid grid-cols-2 gap-4"><label className="text-xs text-slate-500">Earlier year<input aria-label="Earlier graduation year" type="range" min={availableYears[0]||yearMin} max={availableYears.at(-1)||yearMax} value={yearMin} onChange={event=>changeYearRange(Number(event.target.value),yearMax)}/></label><label className="text-xs text-slate-500">Later year<input aria-label="Later graduation year" type="range" min={availableYears[0]||yearMin} max={availableYears.at(-1)||yearMax} value={yearMax} onChange={event=>changeYearRange(yearMin,Number(event.target.value))}/></label></div><p className="text-xs text-slate-500">Selected years: {selectedYears.join(', ')||'Choose a range'}</p>{fieldError('eligible_graduation_years')}</div>
        </Section>
...
function DateTimePicker({id,title,value,error,onChange}:{id:string;title:string;value:string;error?:string;onChange:(value:string)=>void}){
  ...
  return <fieldset className="dw-datetime"><legend>{title}<b className="ml-1 text-rose-600">*</b></legend><div className="grid grid-cols-[1fr_92px_82px_82px] gap-2"><label className="text-xs text-slate-500">Date<input id={id} aria-label={`${title} date`} className={input} type="date" value={date} onChange={event=>set(event.target.value)}/></label><label className="text-xs text-slate-500">Hour<select aria-label={`${title} hour`} className={input} value={parts.hour} onChange={event=>set(date,event.target.value)}>{HOURS.map(hour=><option key={hour}>{hour}</option>)}</select></label><label className="text-xs text-slate-500">Minute<input aria-label={`${title} minute`} className={input} type="number" inputMode="numeric" min={0} max={59} step={1} value={Number(parts.minute)} onChange={event=>{const minute=event.target.value;if(minute===''||Number(minute)<0||Number(minute)>59)return;set(date,parts.hour,minute.padStart(2,'0'));}}/></label><label className="text-xs text-slate-500">AM / PM<select aria-label={`${title} AM / PM`} className={input} value={parts.ampm} onChange={event=>set(date,parts.hour,parts.minute,event.target.value)}><option>AM</option><option>PM</option></select></label></div>{error&&<FieldError id={`${id}-error`} message={error}/>}</fieldset>;
}
