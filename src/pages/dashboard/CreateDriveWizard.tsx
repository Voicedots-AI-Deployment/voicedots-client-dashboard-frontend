...
export default function CreateDriveWizard({ programs, drive, onCancel, onSaved }:{programs:Program[];drive?:Drive|null;onCancel:()=>void;onSaved:(message:string)=>void}) {
  ...
  useEffect(() => { collegeApi.get<AgentLibrary>('agents').then(setLibrary).catch(error => setError(collegeError(error))); collegeApi.get<{graduation_years:number[]}>('academic-catalog').then(value => setAvailableYears(value.graduation_years || [])).catch(error => setError(collegeError(error))); }, []);
  useEffect(() => {
    if (!drive) return;
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (saved) {
        ...
        if (typeof saved.creationKey === 'string' && saved.creationKey.length >= 16) setCreationKey(saved.creationKey);
      }
  ...
  useEffect(() => {
    ...
    localStorage.setItem(DRAFT_KEY,JSON.stringify({form,selection,rounds,programIds,departments,years,step,creationKey,saved_at:new Date().toISOString()}));
  ...
  function add(value:string) {
    ...
    if (custom) {
      if (selection.some(item => item.agent_id === custom.id)) return;
      const openSlot = selection.findIndex(item => !item.agent_id);
      if (openSlot >= 0) {
        setSelection(current => current.map((item, index) => index === openSlot ? { ...item, agent_id: custom.id! } : item));
        return;
      }
      const freeTrack = ['hr','domain','industry','manager'].find(track => !selection.some(item => item.track === track));
      if (!freeTrack) return;
      setSelection(current => [...current, { track: freeTrack, agent_id: custom.id! }]);
      return;
    }
    const track=defaultTrack||value;
    if (selection.some(item=>item.track===track)) return;
    setSelection(current=>[...current,{track,agent_id:null}]);
  }
 ...
 {step===2&&<div className="space-y-4"><p className="text-sm text-slate-500">Selected order is the interview order. The interview ends after the final configured round.</p>{selection.map((item,index)=><div className="flex flex-wrap items-center gap-2 rounded-xl border p-3" key={item.track}><strong className="min-w-52 flex-1 text-sm">{index+1}. {selectionName(item,library)}</strong><button type="button" className={button} disabled={!index} onClick={()=>move(index,-1)} aria-label={`Move round ${index+1} up`}><ChevronUp size={16}/></button><button type="button" className={button} disabled={index===selection.length-1} onClick={()=>move(index,1)} aria-label={`Move round ${index+1} down`}><ChevronDown size={16}/></button><button type="button" className={button} onClick={()=>setSelection(current=>current.filter((_,position)=>position!==index))}><Trash2 size={16}/>Remove</button></div>)}<label className="block text-sm">Add agent<select className={input} value="" disabled={!library||selection.length>=4} onChange={event=>add(event.target.value)}><option value="">Select agent</option>{library&&[...library.tracks.map(track=>({...track,id:'',name:track.default_profile.name,role:track.default_profile.role})),...library.agents].filter(agent=>!selection.some(item=>agent.id?item.agent_id===agent.id:!item.agent_id&&item.track===agent.track)).map(agent=>{const custom=Boolean(agent.id);const optionValue=custom?`custom:${agent.id}`:`default:${agent.track}`;return <option key={optionValue} value={optionValue}>{custom ? agent.role : `${agent.name} — ${agent.role}`}</option>})}</select></label></div>}
      ...
      {step===4&&<EligibilityStep
        programs={programs} programIds={programIds}
        setProgramIds={value=>{setProgramIds(value);setDifficultyConfirmed(false)}}
        departments={departments} setDepartments={setDepartments}
        availableDepartments={availableDepartments} availableYears={availableYears}
        years={years} setYears={value=>{setYears(value);setDifficultyConfirmed(false)}}
        minCgpa={form.min_cgpa} setMinCgpa={value=>update('min_cgpa',value)}
        ambiguousDifficulty={ambiguousDifficulty} preview={preview}
        previewBusy={previewBusy} refresh={()=>void loadPreview()}
      />}
