...
function DateTimePicker({id,title,value,error,onChange}:{id:string;title:string;value:string;error?:string;onChange:(value:string)=>void}){
  ...
  return <fieldset className="dw-datetime"><legend>{title}<b className="ml-1 text-rose-600">*</b></legend><div className="grid grid-cols-[1fr_92px_82px_82px] gap-2"><label className="text-xs text-slate-500">Date<input id={id} aria-label={`${title} date`} className={input} type="date" value={date} onChange={event=>set(event.target.value)}/></label><label className="text-xs text-slate-500">Hour<select aria-label={`${title} hour`} className={input} value={parts.hour} onChange={event=>set(date,event.target.value)}>{HOURS.map(hour=><option key={hour}>{hour}</option>)}</select></label><label className="text-xs text-slate-500">Minute<select aria-label={`${title} minute`} className={input} value={parts.minute} onChange={event=>set(date,parts.hour,event.target.value)}>{MINUTES.map(minute=><option key={minute}>{minute}</option>)}</select></label><label className="text-xs text-slate-500">AM / PM<select aria-label={`${title} AM / PM`} className={input} value={parts.ampm} onChange={event=>set(date,parts.hour,parts.minute,event.target.value)}><option>AM</option><option>PM</option></select></label></div>{error&&<FieldError id={`${id}-error`} message={error}/>}</fieldset>;
}
