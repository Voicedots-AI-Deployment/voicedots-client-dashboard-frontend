  ...
  return <section className="drive-wizard space-y-6 text-slate-900 dark:text-white">
    ...
    <main className={`${panel} dw-main`}>
      ...
      {step===5&&<div className="dw-content dw-review">
        ...
        <ReviewCard title="Interview schedule" edit={()=>goTo(1)} lines={[`Starts ${formatLocal(form.window_start,collegeTimezone)}`,`Ends ${formatLocal(form.window_end,collegeTimezone)}`,`Interview time zone: IST (${collegeTimezone})`,`Duration ${form.duration} minutes · ${form.max_attempts} attempts`,`${displayName(form.difficulty_tier)} difficulty`,form.application_deadline?`Applications close ${formatDateOnly(form.application_deadline)}`:'No application deadline',form.drive_date?`Drive event date ${formatDateOnly(form.drive_date)}`:'No separate drive event date']}/>
