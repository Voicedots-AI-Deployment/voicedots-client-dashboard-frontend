...
function DateTimePicker({id,title,value,error,onChange}:{id:string;title:string;value:string;error?:string;onChange:(value:string)=>void}){
  ...
  const setMinute=(raw:string)=>{const digits=raw.replace(/\D/g,'').slice(0,2);editingMinute.current=true;if(!digits){setMinuteText('00');onChange(joinDateTime(date,parts.hour,'00',parts.ampm));return;}if(digits.length===1){const normalized=`0${digits}`;setMinuteText(normalized);onChange(joinDateTime(date,parts.hour,normalized,parts.ampm));return;}const number=Math.min(59,Number(digits));const normalized=String(number).padStart(2,'0');setMinuteText(normalized);onChange(joinDateTime(date,parts.hour,normalized,parts.ampm));};
  ...
  return <fieldset className="dw-datetime"><legend>{title}<b className="ml-1 text-rose-600">*</b></legend><div className="grid grid-cols-[1fr_92px_82px_82px] gap-2"><label className="text-xs text-slate-700">Date<input id={id} aria-label={`${title} date`} className={`${input} text-slate-950 [color-scheme:light]`} type="date" value={date} onChange={event=>set(event.target.value)}/></label><label className="text-xs text-slate-700">Hour<select aria-label={`${title} hour`} className={`${input} text-slate-950`} value={parts.hour} onChange={event=>set(date,event.target.value)}>{HOURS.map(hour=><option key={hour}>{hour}</option>)}</select></label><label className="text-xs text-slate-700">Minute<input aria-label={`${title} minute`} className={`${input} text-slate-950`} type="text" inputMode="numeric" placeholder="00" maxLength={2} value={minuteText} onBlur={finishMinute} onChange={event=>setMinute(event.target.value)}/></label><label className="text-xs text-slate-700">AM / PM<select aria-label={`${title} AM / PM`} className={`${input} text-slate-950`} value={parts.ampm} onChange={event=>set(date,parts.hour,parts.minute,event.target.value)}><option>AM</option><option>PM</option></select></label></div>{error&&<FieldError id={`${id}-error`} message={error}/>}</fieldset>;
}
