...
export function DriveInterviewSetup({ selection, setSelection, source, setSource, questions, setQuestions, form, busy, onGenerating }: {
  ...
  function add(key: string) {
    ...
    if (agent.id) {
      if (selection.some(item => item.agent_id === agent.id)) return;
      const openSlot = selection.findIndex(item => !item.agent_id);
      if (openSlot >= 0) { setSelection(selection.map((item, index) => index === openSlot ? { ...item, agent_id: agent.id! } : item)); return; }
    }
    const slot = agent.id ? tracks.find(t => !occupied.includes(t)) : agent.track;
    if (!slot || occupied.includes(slot)) return;
    setSelection([...selection, { track: slot, agent_id: agent.id || null }]);
  }
