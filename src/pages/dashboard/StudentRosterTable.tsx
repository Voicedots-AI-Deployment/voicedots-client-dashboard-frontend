...
export default function StudentRosterTable({students,total,offset,busy,onPage,onSort,sorting,onEdit,onDelete}:{students:CollegeStudent[];total:number;offset:number;busy:boolean;onPage:(offset:number)=>void;onSort:(value:SortingState)=>void;sorting:SortingState;onEdit:(student:CollegeStudent)=>void;onDelete:(student:CollegeStudent)=>void}) {
 const columns=useMemo<ColumnDef<CollegeStudent>[]>(()=>[
  ...
  {id:'actions',header:'Actions',enableSorting:false,cell:({row})=><div className="flex flex-wrap gap-2"><button className="rounded-lg border px-3 py-2" onClick={()=>onEdit(row.original)}>Edit</button><button className="rounded-lg border border-rose-200 px-3 py-2 text-rose-700" onClick={()=>onDelete(row.original)}>Delete</button></div>},
 ],[onEdit,onDelete]);
