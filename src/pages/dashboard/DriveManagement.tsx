...
function DepartmentView({ data }: { data: Data }) {
  ...
  return (
    <div className="space-y-5">
      ...
      {selected&&<div className="fixed inset-0 z-50 bg-slate-950/40" onMouseDown={e=>{if(e.target===e.currentTarget)setSelected(null)}}><aside className="ml-auto h-full w-full max-w-md overflow-y-auto bg-white p-6 shadow-2xl dark:bg-slate-950"><div className="flex justify-between"><h3 className="text-xl font-bold">{String(selected.department_code)} students</h3><button className={btn} onClick={()=>setSelected(null)}>Close</button></div><div className="mt-5 divide-y rounded-xl border">{(selected.students||[]).map((student: Data)=><div className="p-3 text-sm" key={String(student.student_id)}><div className="font-medium">{String(student.full_name)}</div><div className="text-slate-500">{String(student.roll_number||"")} · {String(student.score)} · {displayName(String(student.readiness||"not assessed"))}</div></div>)}{!(selected.students||[]).length&&<div className="p-3 text-sm text-slate-500">No released scored students.</div>}</div></aside></div>}
    </div>
  );
}
